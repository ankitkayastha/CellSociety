...
public class NeighborFactory {
	...
	private int[] getRowDelta(int index) {
		if ((currentShape == TRIANGLE || currentShape == SQUARE)
				&& (currentSim == FIRE || currentSim == WATOR || currentSim == SUGAR)) {
	...
	private int[] getIndexDelta(int index) {
		if ((currentShape == TRIANGLE || currentShape == SQUARE)
			...
			return new int[] { index - numCols, index + ((index % 2) - 1) * numCols - 1,
					index + ((index % 2) - 1) * numCols + 1, index + numCols, index + (index % 2) * numCols - 1,
					index + (index % 2) * numCols + 1 };
		}
		return null;
	}
